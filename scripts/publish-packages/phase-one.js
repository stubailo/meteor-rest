...
`);
  ...
  Please commit or stash before publishing.`);
    ...
    packagesToRepublish.forEach((pkg) => {
      const packageJsFile = `packages/${dir(pkg)}/package.js`;
...
What should the new version be? ${defaultVersionDisplay}`) || defaultNewVersion;
      ...
      // script yet.
      fs.readdirSync("packages/").forEach(otherPkgDir => {
        const otherPackageJsFile = `packages/${otherPkgDir}/package.js`;
